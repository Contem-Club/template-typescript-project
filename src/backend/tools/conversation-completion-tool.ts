...
export const ConversationCompletionTool = tool({
  ...
  parameters: z.object({
    reason: z.string().describe('The reason why the conversation appears to be complete'),
    confidence: z.number().min(0).max(1).describe('Confidence level (0-1) that the conversation is actually complete'),
    lastUserMessage: z.string().describe('The last message from the user that triggered this detection'),
  }),
