...
describe('ConversationCompletionTool', () => {
  it('should have correct description and parameters', () => {
    expect(ConversationCompletionTool.description).toContain('detect when a conversation appears to be naturally finished');
  ...
  it('should execute with high confidence completion', async () => {
    ...
    expect(result).toContain('Conversation marked as complete with high confidence');
  ...
  it('should log completion data', async () => {
    ...
    expect(consoleSpy).toHaveBeenCalledWith('Conversation completion detected:', expect.objectContaining({
      reason: 'Test reason',
      confidence: 0.8,
      lastUserMessage: 'Test message',
